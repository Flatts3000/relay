...
export function PublicHeader() {
  ...
  const linkClass = (path: string) =>
    `inline-flex items-center min-h-[44px] px-3 py-2 text-sm font-medium rounded-lg transition-colors ${
  ...
  return (
    <>
      {/* A-3: every page rendering this chrome gets a skip link. It lived in
          HomePage alone, so /directory, /privacy, /security and /terms each
          made a keyboard user tab the whole header on every load. */}
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[60] focus:inline-flex focus:items-center focus:min-h-[44px] focus:px-4 focus:py-2 focus:bg-primary-600 focus:text-white focus:rounded-lg focus:text-sm focus:font-medium"
      >
        {t('skipToContent')}
      </a>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-center h-16 gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/login"
                className="hidden md:inline-flex items-center min-h-[44px] px-3 py-2 text-sm font-medium text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-50"
              >
                {t('navigation.login')}
              </Link>
              <button
