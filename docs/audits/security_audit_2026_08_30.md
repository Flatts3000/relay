...
| **SEC-1a** the CSP blanked the deck         | **Found while reviewing the fix, and fixed.** The first policy applied site-wide, and `/deck` is one self-contained file by design: an inline `<script>` for its slide engine and base64 `data:` webfonts, so it works offline as an attachment. `script-src 'self'` and `font-src 'self'` blocked both, and the failure was silent and total - 15 slides, none activated, a blank page for anyone opening relayfunds.org/deck. `/deck` now carries its own policy allowing the inline script **by hash** rather than `'unsafe-inline'`, plus `data:` fonts. `deploy/check-nginx-parity.mjs` recomputes that hash in CI, so a deck rebuild that changes the script fails the build instead of shipping a blank deck to a funder. The hash must be taken over LF-normalised text: the HTML parser normalises CRLF before hashing, so raw bytes from a Windows checkout give a hash the browser never agrees with. |
